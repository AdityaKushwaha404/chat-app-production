...
const Register = () => {
  ...
  const onSubmit = () => {
    if (!canSubmit) {
      Alert.alert("Missing info", "Please enter name, email and password (min 6 chars).");
      return;
    }

    (async () => {
      try {
        // pass name, email, password to match AuthContext implementation
        await auth.signUp(name, email, password);
      } catch (e: any) {
        Alert.alert("Sign up failed", e.message || "Unable to register");
      }
    })();
  };
        ...
        <View style={styles.card}>
          ...
          <View style={{ marginTop: spacingY._25 }}>
            <Input
              ...
              returnKeyType="next"
              icon={<Ionicons name="person-outline" size={20} color={colors.neutral600} />}
            />
